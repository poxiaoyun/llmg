...
前端 UI 二开前必须先阅读 `../../docs/codex/frontend-ui-dev-index.md`。`web/default` 是默认前端基线与对比对象，不作为二开 UI 的主改造目录；二开界面应放在 `web/llmg`。只有为接入新主题所需的共享配置、选择器入口或上游兼容修复，才允许触碰本目录，并应在交付说明中明确说明。
